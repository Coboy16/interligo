...
class LoggerUtil {
  ...
  static void logDatabaseEvent(String table, String action, {dynamic data}) {
    if (kDebugMode) {
      final timestamp = DateTime.now().toIso8601String();
      dev.log(
        '[$timestamp] DB_LOG: Table [$table] | Action: $action | Data: ${data.toString()}',
        name: 'Interligo.Database',
      );
    }
  }
